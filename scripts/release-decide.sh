...
# 訊きたいのは「公開済みか」。draft は未リリースとして扱う: publish はビルドが作った
# draft を前にしてこの判定を呼ぶ。失敗した run が残した draft も未リリースと答えるが、
# mullion の build はそれを再利用せず preflight で止める (中途半端なアセットを
# 公開しないため。消してから再実行する)。このエンドポイントは draft に 404 を返す (ドキュメントは
